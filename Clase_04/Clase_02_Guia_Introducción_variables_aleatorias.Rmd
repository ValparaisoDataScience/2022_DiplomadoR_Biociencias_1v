---
title: "Guía de trabajo Variables Aleatorias"
...
Elabore un archivo o file con extensión **.R** usando la barra de herramientas de **Rstudio**. Selecciona **File \> New file \> R script**. Utilice el siguiente ejemplo para completar la información de **metadatos** del script.
    ...
    # ----------------------------------------------------------  
    # Clase 02 - Script Variables Aleatorias.   
    # Su nombre. 
    # Fecha de hoy. 
    # DBT 845 - Investigación reproducible y análisis de datos biotecnológicos con R. 
    # ----------------------------------------------------------  

Luego, guarde inmediatamente su script como **script_1_nombre_apellido.R**.

### **Ejercicio 2.** **Configuración de librerías**

Debajo del **metadatos** habilite las siguientes librerías la función **library()**: **readr**, **MASS** **Rlab**. Note que las librerías **base**, **stats**, **utils** y **graphics**.
...
read_delim()*** de la librería **readr**. Explore el set de datos algunas funciones de la librería **{utils}** (**head()**, **tail**, **str())** y de la librería **{base}** (**summary**).
...
Una empresa exportadora de productos biotecnológicos implementa un sistema de inspección visual de los productos, clasificándo cada uno como defectuoso **(D)** y no defectuoso **(ND)**.
...
Diversas investigaciones en lavaduras han determinado la tasa de mutación espontanea, un parámetro importante que alimenta la evolución y la diversidad genética entre entre especies. En este ejercicio simularemos el número de mutaciones espontaneas en *Schizosaccharomyces pombe* tratando de replicar los resultados observados por [Behringer and Hall, 2016](https://academic.oup.com/g3journal/article/6/1/149/6027351) en la figura 3.

**a)** Cree un objeto denominado levadura y genere una muestra aletoria de mutaciones en 326 lineas de levadura usando la función **rnegbin(m, mu, theta)**. Considere los siguientes argumentos: n=326, mu = 4.13, thetha = 30. Explore los datos con las funciones **head()**, y **summary()**.
...
**b)** Usando la función **hist()** y el argumento *probability = TRUE* replique la siguiente figura para observar la densidad del número de mutaciones espontanea.

```{r}
hist(levadura, probability = TRUE, col="lightblue", main = "Densidad del número de mutaciones.", xlab = "Número de mutaciones espontáneas en levadura.")
```
