---
title: "Clase 18 Introducción a Modelos Lineales Generales"
...
grid.arrange(microplot1+My_Theme, microplot2+My_Theme, ncol=2, nrow =1)
```
...
summary(reg_mussel)$coef %>% kable(digits = 2)
...
summary(reg_mussel_2)$coef %>% kable(digits=2)
...
```{r}
mod_lineal <- lm(Maturation ~ Gonad, data = maduracion)
summary(mod_lineal)$coef %>% kable(digits = 2)
...
mod_logit <- glm(Maturation ~ Gonad, 
                family= binomial, data = maduracion)
summary(mod_logit)$coef %>% kable(digits = 2)
```
...
mod_logit_mult <- glm(Maturation ~ Gonad + 
                      ...
                      data = maduracion)
summary(mod_logit_mult)$coef %>% kable(digits = 2)
```
...
mod_nulo <- glm(Maturation ~ 1, 
                family= binomial, data = maduracion)
summary(mod_nulo)$coef %>% kable(digits = 2)
```
